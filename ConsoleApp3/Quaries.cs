    ...
    internal class Queries
    {
        ...
        public string cat()
        {
            ...
            for (int i = 0; i <= max_cat; i++)
            {
                ...
                {
                    random_breed = faker.Random.Number(1, max_breed);   //Создаём случайное число от 1 до максимального числа breed
                }

                int random_pride = faker.Random.Number(1, max_pride); //пока нужный не найдём
                while (non_pride.IndexOf($"({random_pride})") != -1)
                {
                    random_pride = faker.Random.Number(1, max_pride);   //создаём случайное число от 1 до максимального числа прайда
                }

                r = r + ($"('{nick[i]}','{random_breed}','{random_pride}','{faker.Random.Number(max_age) + 1}')");//создаём кота из всех ранее рандомно сгенерированных переменных
                if (i != max_cat) r = r + ",";
            }
            return r;
        }
        ...
        {
            int rand2 = faker.Random.Number(2, 3);//случайно соединяем ранее созданных котов и хозяев 
            int rand3 = faker.Random.Number(5, 10);

            string r = "INSERT INTO cat_owner(cat_id,owner_id) VALUES "; //Вписываем их в базу данных
